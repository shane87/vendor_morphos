...
func init() {
	android.RegisterModuleType("morphos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = morphosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = morphosExpandVariables(ctx, dep_file)
	...
	cmd := morphosExpandVariables(ctx, String(g.properties.Cmd))
