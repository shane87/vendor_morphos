...
function setup_vendor() {
    ...
    export MORPHOS_ROOT="$3"
    if [ ! -d "$MORPHOS_ROOT" ]; then
        echo "\$MORPHOS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$MORPHOS_ROOT/$OUTDIR" ]; then
        mkdir -p "$MORPHOS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$MORPHOS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$MORPHOS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$MORPHOS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$MORPHOS_ROOT"/vendor/morphos/build/tools/smali/baksmali.jar
        export SMALIJAR="$MORPHOS_ROOT"/vendor/morphos/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$MORPHOS_ROOT"/vendor/morphos/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$morphos_ROOT"/vendor/morphos/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$MORPHOS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$MORPHOS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$MORPHOS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$MORPHOS_ROOT"/vendor/morphos/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$MORPHOS_ROOT"/"$OUTDIR"/radio
