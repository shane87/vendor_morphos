...
def get_from_github(device):
        ...
        githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:shane87+in:name+fork:true" % device)
        if githubauth:
...
def checkdeps(repo_path):
    ...
    if ((len(losdeps) + len(morphosdeps)) < 1):
        ...
        if (len(morphosdeps) > 0):
            ran_checkdeps_on.append("HAS_MORPHOS_DEPS:\t" + repo_path)
            morphosdeps = morphosdeps[0]
            morphosdeps = open(morphosdeps, 'r')
            morphosdeps = json.loads(morphosdeps.read())
            for dep in morphosdeps:
                try:
                    branch = dep['branch']
                except:
                    branch = None
